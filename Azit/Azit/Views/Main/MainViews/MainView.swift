...
struct MainView: View {
    ...
    @State private var isSendFriendStoryToast = false // 상대방 게시물에 메시지를 전송했는가? (Toast Message)
            ...
            NavigationStack() {
                NavigationStack() {
                    ZStack {
                        // 메인 화면일 때 타원 뷰
                        if isMainExposed {
                            RotationView(isMainExposed: $isMainExposed, isMyModalPresented: $isMyModalPresented, isFriendsModalPresented: $isFriendsModalPresented, isDisplayEmojiPicker: $isDisplayEmojiPicker, isPassed24Hours: $isPassed24Hours, isSendFriendStoryToast: $isSendFriendStoryToast, isTappedWidget: $isTappedWidget, isAnimatingForStroke: $isAnimatingForStroke)
                                .frame(width: 300, height: 300)
                                .zIndex(isMyModalPresented
                                        || isFriendsModalPresented
                                        || isDisplayEmojiPicker
                                        || isTappedWidget ? 2 : 1)
                            MainTopView(isMainExposed: $isMainExposed, isSendFriendStoryToast: $isSendFriendStoryToast)
                            //.ignoresSafeArea(.keyboard)
                            // 맵 화면일 때 맵 뷰
                        } else {
                            MapView(isMainExposed: $isMainExposed, isMyModalPresented: $isMyModalPresented, isFriendsModalPresented: $isFriendsModalPresented, isDisplayEmojiPicker: $isDisplayEmojiPicker, isPassed24Hours: $isPassed24Hours, isSendFriendStoryToast: $isSendFriendStoryToast, isAnimatingForStroke: $isAnimatingForStroke)
                                .zIndex(isMyModalPresented
                                        || isFriendsModalPresented
                                        || isDisplayEmojiPicker
                                        || isTappedWidget ? 2 : 1)
                    ...
                    .toast(isPresenting: $isSendFriendStoryToast, alert: {
                        AlertToast(displayMode: .banner(.pop), type: .systemImage("envelope.open", Color.white), title: "전송 완료", style: .style(backgroundColor: .subColor1, titleColor: Color.white))
                    })
                    //        .gesture (
                    //            DragGesture()
                    ...
                    .onChange(of: scenePhase) {
                        ...
                        @unknown default:
                            ...
                            if isShowingMessageView {
                                MessageView(isShowToast: $isShowToast, isShowingMessageView: $isShowingMessageView)
                            }
